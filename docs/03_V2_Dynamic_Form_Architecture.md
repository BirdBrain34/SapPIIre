...
4. The `form_template_notifications` table stores template and field lifecycle events consumed by `FormTemplateNotificationService` (`lib/services/form_template_notification_service.dart`) on both mobile and web clients via Realtime subscriptions. Client-side deduplication (`_processedIds` in the service, `_uiProcessedNotifIds` in `manage_info_screen.dart`) prevents duplicate toast rendering when Realtime delivers the same row in both an initial snapshot and subsequent INSERT event. A 1.2s debounce window accumulates multiple field-level notifications from a single save operation into one aggregated toast.
